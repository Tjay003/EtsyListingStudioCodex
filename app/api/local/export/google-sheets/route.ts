  ...
  try {
    ...
    for (const instanceId of requestedIds) {
      ...
      const itemPayload = {
        ...
        shop_name: (settings.shop_name || "").trim() || path.basename(root) || "",
        workspace_name: path.basename(root) || "",
        folder_name: product.folderName,
        sheet_name: (settings.shop_name || "").trim() || path.basename(root) || "Listings",
        target_sheet: (settings.shop_name || "").trim() || path.basename(root) || "Listings",
