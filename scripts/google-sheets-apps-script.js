...
function doPost(e) {
  ...
  try {
    ...
    return ContentService.createTextOutput(
      JSON.stringify({ success: false, error: "Server busy, could not obtain lock: " + err.toString() })
  ...
  try {
    ...
    try {
      ...
      return ContentService.createTextOutput(
        JSON.stringify({ success: false, error: "Invalid JSON format: " + parseErr.message })
      ).setMimeType(ContentService.MimeType.JSON);
    }
...
function syncSingleItem(sheet, item) {
  ...
  // 1. Update existing row in place
  ...
  // 2. Insert in strict numerical order (e.g. 6 above 7)
...
/**
 * Applies native checkbox validation to Column 4 (Edited Photo Ready?)
 * and dropdown validation to Column 5 (Status) for a specific row
 */
function applyRowValidation(sheet, row) {
  try {
    ...
    // Ignore if validation API is not supported in environment
  }
}

/**
 * Applies column-level formatting and validation across all rows in the sheet
 */
function applyColumnFormatting(sheet) {
  ...
  try {
    ...
    // Alignment
    sheet.getRange(2, 1, numRows, 1).setHorizontalAlignment("center"); // ID
    sheet.getRange(2, 4, numRows, 1).setHorizontalAlignment("center"); // Checkbox
    sheet.getRange(2, 5, numRows, 1).setHorizontalAlignment("center"); // Status
    sheet.getRange(2, 10, numRows, 2).setHorizontalAlignment("right");  // Prices
  } catch (err) {
    // Ignore formatting errors if running outside standard spreadsheet
  }
}
