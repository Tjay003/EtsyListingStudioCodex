...
// Listen for messages from the popup script. Guarding this avoids duplicate replies
// when the popup reinjects the content script in the same tab.
if (!window.__ALI_ETSY_SCRAPER_LISTENER__) {
  window.__ALI_ETSY_SCRAPER_LISTENER__ = true;
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    } else if (request.action === "loadDescriptionImages") {
      loadDescriptionImages().then(productData => {
        sendResponse({ success: true, data: productData });
      }).catch(error => {
        sendResponse({ success: false, error: error.message });
      });
...
async function loadDescriptionImages() {
  const originalX = window.scrollX;
  const originalY = window.scrollY;
  const scroller = document.scrollingElement || document.documentElement;
  const pageHeight = Math.max(scroller.scrollHeight || 0, document.body.scrollHeight || 0);
  const scrollTargets = [];
  const anchor = getDescriptionAnchor();
  ...
  [0.55, 0.7, 0.85, 1].forEach(ratio => {
    scrollTargets.push(Math.max(0, Math.floor(pageHeight * ratio) - window.innerHeight));
  });

  let latestData = await scrapePage();
  for (const targetY of scrollTargets) {
    if (latestData.description_images.length > 0) break;
    window.scrollTo({ left: originalX, top: targetY, behavior: "auto" });
    await waitFor(800);
    latestData = await scrapePage();
  }
